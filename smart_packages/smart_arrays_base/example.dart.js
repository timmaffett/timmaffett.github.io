...
(function dartProgram() {
  ...
  var A = {JS_CONST: function JS_CONST() {
    ...
    Array2D_getSubmatrix(matrix, row1, row2, col1, col2, negLevels) {
      var size, nrows, ncols, endRow, firstRow, firstCol, result, i, destrow, inputRow, destrow0, resultRow, t2, t3, j, t4, curval,
        t1 = matrix.length;
      if (0 >= t1)
        return A.ioore(matrix, 0);
      size = matrix[0].length;
      if (row1 === 0 && row2 === t1 && col1 === 0 && col2 === t1)
        return matrix;
      if (row1 > t1 || row2 > t1 || col1 > size || col2 > size)
        throw A.wrapException(new A._Exception("CompressedData2D: Submatrix out of bounds"));
      nrows = Math.abs(row2 - row1);
      ncols = Math.abs(col2 - col1);
      if (row1 > row2) {
        endRow = row1;
        firstRow = row2;
      } else {
      ...
      firstCol = col1 > col2 ? col2 : col1;
      result = A.List_List$filled(nrows, new Float64Array(ncols), type$.Float64List);
      for (i = firstRow, destrow = 0; i < endRow; ++i, destrow = destrow0) {
        if (!(i < t1))
          return A.ioore(matrix, i);
        inputRow = matrix[i];
        destrow0 = destrow + 1;
        if (!(destrow < nrows))
          return A.ioore(result, destrow);
        resultRow = result[destrow];
        for (t2 = resultRow.length, t3 = inputRow.length, j = 0; j < ncols; ++j) {
          t4 = firstCol + j;
          if (!(t4 < t3))
            return A.ioore(inputRow, t4);
          curval = inputRow[t4];
          t4 = curval < 0;
          if (t4)
            curval = 0;
          if (!(j < t2))
            return A.ioore(resultRow, j);
          resultRow[j] = curval;
        }
      }
      return result;
    ...
    main() {
      var i, row, t1, t2, k, t3, row100, col100, sub_matrix, minmax,
        _s31_ = " occurred at submatrix row/col ",
        matrix = A.List_List$filled(123, new Float64Array(234), type$.Float64List);
      for (i = 0; i < 123; ++i) {
        row = matrix[i];
        for (t1 = "" + i + ".", t2 = row.length, k = 0; k < 234; ++k) {
          t3 = A.double_parse(t1 + k);
          if (!(k < t2))
            return A.ioore(row, k);
          row[k] = t3;
        }
      }
      row100 = A.Array2D_getRow(matrix, 101);
      col100 = A.Array2D_getColumn(matrix, 105);
      sub_matrix = A.Array2D_getSubmatrix(matrix, 100, 110, 200, 220, false);
      minmax = A.Array2D_getMinMax(sub_matrix);
      A.print("matrix row 100 = " + A.S(row100));
      A.print("matrix col 100 = " + A.S(col100));
      for (t1 = sub_matrix.length, i = 0; i < t1; ++i)
        A.printString("submatrix row " + i + "=" + A.S(sub_matrix[i]));
      A.print("Minimum value " + A.S(minmax.minValue) + _s31_ + (" " + minmax.minValueIndexRow + " / " + minmax.minValueIndexCol));
      A.print("Maximum value " + A.S(minmax.maxValue) + _s31_ + (" " + minmax.maxValueIndexRow + " / " + minmax.maxValueIndexCol));
    }
  },
  J = {
  ...
  A.NativeTypedArrayOfDouble.prototype = {$isIterable: 1};
  ...
  (function inheritance() {
    ...
    _inheritMany(A.Object, [A.JS_CONST, J.Interceptor, J.ArrayIterator, A.Error, A.ListIterator, A.FixedLengthListMixin, A.Closure, A.Rti, A._FunctionParameters, A._Type, A.ListBase, A._Exception, A.FormatException, A.Null, A.StringBuffer, A.MinMax]);
  ...
  A._Universe_addRules(init.typeUniverse, JSON.parse('{"PlainJavaScriptObject":"LegacyJavaScriptObject","UnknownJavaScriptObject":"LegacyJavaScriptObject","JavaScriptFunction":"LegacyJavaScriptObject","JSBool":{"bool":[],"TrustedGetRuntimeType":[]},"JSNull":{"TrustedGetRuntimeType":[]},"JSArray":{"Iterable":["1"]},"JSUnmodifiableArray":{"JSArray":["1"],"Iterable":["1"]},"JSNumber":{"double":[],"num":[]},"JSInt":{"double":[],"int":[],"num":[],"TrustedGetRuntimeType":[]},"JSNumNotInt":{"double":[],"num":[],"TrustedGetRuntimeType":[]},"JSString":{"String":[],"TrustedGetRuntimeType":[]},"NativeTypedArray":{"JavaScriptIndexingBehavior":["1"]},"NativeTypedArrayOfDouble":{"ListBase":["double"],"JavaScriptIndexingBehavior":["double"],"Iterable":["double"],"FixedLengthListMixin":["double"]},"NativeFloat64List":{"ListBase":["double"],"Float64List":[],"JavaScriptIndexingBehavior":["double"],"Iterable":["double"],"FixedLengthListMixin":["double"],"TrustedGetRuntimeType":[],"ListBase.E":"double"},"double":{"num":[]},"Float64List":{"Iterable":["double"]}}'));
