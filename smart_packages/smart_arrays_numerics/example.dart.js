...
(function dartProgram() {
  ...
  var A = {JS_CONST: function JS_CONST() {
    ...
    main() {
      var exception, transposed,
        nu = new A.Numerics(),
        t1 = type$.JSArray_double,
        t2 = type$.JSArray_List_double,
        mat = A._setArrayType([A._setArrayType([1, 1, 1, 0], t1), A._setArrayType([0, 3, 1, 2], t1), A._setArrayType([2, 3, 1, 0], t1), A._setArrayType([1, 0, 2, 1], t1)], t2),
        inverse = null, identity = null;
      try {
        inverse = nu.invMatrix$1(mat);
        identity = nu.dotMM$2(mat, inverse);
        A.print("inverse=" + A.S(inverse));
        A.print("identity=" + A.S(identity));
        A.print("diagonal inverse=" + A.S(nu.getDiag$1(inverse)));
      } catch (exception) {
        A.print("not invertible: " + A.S(mat));
      }
      transposed = nu.transpose$1(A._setArrayType([A._setArrayType([1, 1, 1], t1), A._setArrayType([0, 3, 1], t1), A._setArrayType([2, 3, 1], t1), A._setArrayType([1, 0, 2], t1)], t2));
      A.print("transposed=" + A.S(transposed));
      A.print("diagonal transposed=" + A.S(nu.getDiag$1(transposed)));
    }
  },
  J = {
  ...
  A.Numerics.prototype = {
    dotVV$2(vec1, vec2) {
      ...
      t1 = vec1[3];
      if (3 >= vec2.length)
        return A.ioore(vec2, 3);
      ret = t1 * vec2[3];
      for (i = 2; i >= 1; i -= 2) {
        i1 = i - 1;
        ret += vec1[i] * vec2[i] + vec1[i1] * vec2[i1];
      }
      return i === 0 ? ret + vec1[0] * vec2[0] : ret;
    },
    dotMMsmall$2(x, y) {
      var q, r, ret, _i, k, j, i, foo, bar, k0, t2, t3, t4, woo, j0, i0, t5, t6,
      ...
      if (0 >= q)
        return A.ioore(y, 0);
      r = y[0].length;
      ret = J.JSArray_JSArray$allocateGrowable(4, type$.List_double);
      for (t1 = type$.double, _i = 0; _i < 4; ++_i)
        ret[_i] = A.List_List$filled(r, 0, t1);
      for (k = r - 1, j = q - 2, t1 = q - 1, i = 3; i >= 0; --i) {
        if (!(i < ret.length))
    ...
    dotMMbig$2(x, y) {
      ...
      if (0 >= y.length)
        return A.ioore(y, 0);
      n = y[0].length;
      $A = J.JSArray_JSArray$allocateGrowable(4, type$.List_double);
      for (_i = 0; _i < 4; ++_i)
    ...
    cloneMatrix$1(matrix) {
      ...
      clone = this.createMatrix$3(4, 4, 0);
      for (i = 0; i < 4; ++i)
        for (k = 0; k < 4; ++k) {
          if (!(i < clone.length))
            return A.ioore(clone, i);
          B.JSArray_methods.$indexSet(clone[i], k, matrix[i][k]);
        }
      return clone;
    },
    transpose$1(matrix) {
      var ret, t1, _i, i, A1, A0, j, t2, Bj;
      type$.List_List_double._as(matrix);
      ret = J.JSArray_JSArray$allocateGrowable(3, type$.List_double);
      for (t1 = type$.double, _i = 0; _i < 3; ++_i)
        ret[_i] = A.List_List$filled(4, 0, t1);
      for (i = 3; i >= 1; i -= 2) {
        A1 = matrix[i];
        t1 = i - 1;
        A0 = matrix[t1];
        for (j = 2; j >= 1; --j) {
          ...
          Bj = ret[j];
          B.JSArray_methods.$indexSet(Bj, i, A1[j]);
          B.JSArray_methods.$indexSet(Bj, t1, A0[j]);
          --j;
          if (!(j < t2))
            return A.ioore(ret, j);
          Bj = ret[j];
        ...
        if (j === 0) {
          ...
          B.JSArray_methods.$indexSet(Bj, t1, A0[0]);
        }
      }
      if (i === 0) {
        A0 = matrix[0];
        for (j = 2; j >= 1; --j) {
          ...
          if (!(j < t1))
            return A.ioore(ret, j);
          B.JSArray_methods.$indexSet(ret[j], 0, A0[j]);
          --j;
          if (!(j < t1))
            return A.ioore(ret, j);
          B.JSArray_methods.$indexSet(ret[j], 0, A0[j]);
        ...
        if (j === 0) {
          if (0 >= ret.length)
            return A.ioore(ret, 0);
          B.JSArray_methods.$indexSet(ret[0], 0, A0[0]);
        }
      }
      return ret;
    },
    invMatrix$1(mat) {
      var j, t1, i, i0, v0, t2, kx, Aj, Ij, x, k, Ai, Ii, t3, k0, t4,
        $A = this.cloneMatrix$1(type$.List_List_double._as(mat)),
        I = this.createDiagMatrix$2(4, 1);
      for (j = 0; j < 4; j = k) {
        for (t1 = $A.length, i = j, i0 = -1, v0 = -1; i !== 4; ++i) {
          if (!(i < t1))
            return A.ioore($A, i);
          t2 = $A[i];
          if (!(j < t2.length))
        ...
        for (k = j; k !== 4; ++k) {
          if (!(k < t1))
            return A.ioore(Aj, k);
          B.JSArray_methods.$indexSet(Aj, k, Aj[k] / x);
        }
        for (t2 = Ij.length, k = 3; k !== -1; --k) {
          if (!(k >= 0 && k < t2))
            return A.ioore(Ij, k);
          B.JSArray_methods.$indexSet(Ij, k, Ij[k] / x);
        }
        for (k = j + 1, i = 3; i !== -1; --i)
          if (i !== j) {
            if (!(i >= 0 && i < $A.length))
            ...
            for (k0 = k; k0 !== 4; ++k0) {
              if (!(k0 < t3))
                return A.ioore(Ai, k0);
              t4 = Ai[k0];
              if (!(k0 < t1))
                return A.ioore(Aj, k0);
              B.JSArray_methods.$indexSet(Ai, k0, t4 - Aj[k0] * x);
            }
            for (t3 = Ii.length, k0 = 3; k0 > 0; --k0) {
              if (!(k0 < t3))
                return A.ioore(Ii, k0);
              t4 = Ii[k0];
              if (!(k0 < t2))
                return A.ioore(Ij, k0);
              B.JSArray_methods.$indexSet(Ii, k0, t4 - Ij[k0] * x);
              --k0;
              if (!(k0 < t3))
                return A.ioore(Ii, k0);
              t4 = Ii[k0];
  ...
  (function inheritance() {
    ...
    _inheritMany(A.Object, [A.JS_CONST, J.Interceptor, J.ArrayIterator, A.Error, A.Closure, A.Rti, A._FunctionParameters, A._Type, A.Null, A.StringBuffer, A.Numerics]);
